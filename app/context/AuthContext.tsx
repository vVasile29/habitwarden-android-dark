...
interface AuthProps {
    ...
    onRegister?: (name: string, password: string, age: string, gender: string, profession: string, codeword: string, isDarkGroup: boolean) => Promise<any>;
...
export const API = 'http://10.107.18.195:8080';
...
export const AuthProvider = ({children}: any) => {
    ...
    const register = async (name: string, password: string, age: string, gender: string, profession: string, codeword: string, isDarkGroup: boolean) => {
        try {
            await SecureStore.deleteItemAsync(TOKEN_KEY);
            await SecureStore.deleteItemAsync(USER_KEY);
            axios.defaults.headers.common['Authorization'] = ``;
            return await axios.post(`${AUTH_API}/register`, {name, password, age, gender, profession, codeword, isDarkGroup});
        } catch (e) {
            return e
        }
    }
